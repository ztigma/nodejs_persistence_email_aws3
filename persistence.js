...
/*
decompress("zip/banderas.zip", "public/img/banderas")
  .then((files) => {
    console.log(files);
  })
  .catch((error) => {
...
String.prototype.IMG_DB_PATH = function()
{
	return `db/img_vehicle/DB Vehicles (Images)/${this}`;	
}
...
Function.prototype.ASYNC_TO_STRING = function() {
	return ('async function ' + this.toString() + this.name + "();\n\n").PROPS({ await: 'await' });
}
...
String.prototype.HEAD = function(la) {
	var head =
		`
...
String.prototype.TO_ZIP = function(path) {
	zipdir
		(
			this.toString()
			,
...
*/
/*
zipdir('./public/banderas', { saveTo: './zips/banderas.zip' }, function(err, buffer) {
	console.log("Zip Listo!");
});
*/

String.prototype.EXCEL_TO_PERSISTENCE = function(path, index, callback) {
	var workbook = xlsx.readFile(this + "");
	var sheet_name_list = workbook.SheetNames;
	var json_object = xlsx.utils.sheet_to_json(workbook.Sheets[sheet_name_list[index ? index : 0]]);

	(path ? path : "db/json/no_name.txt").SET_PERSISTENCE(json_object, callback);
}
...
String.prototype.RESERVACION_PATH_FULL = function(mes)
{
	var dt = new Date();
	if(mes < 13)
	{
		var path = ''.RESERVACION_PATH().replaceAll('.xlsx', '.json')
		.PROPS
		({
			mes:mes ? mes : dt.MES()
			,
